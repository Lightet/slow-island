...
- **摸摸手**：点一下动物，或按住轻轻来回摸。手指轻压、手腕来回摆动，猫会闭眼歪头蹭手，脚底保持在原位；手移远后会停下。
...
- `assets/companions.js` / `.css`：表情、道具、嘴边锚点与触摸交互。
- `assets/animal-atlas.png`：透明动物图集；`contented-faces.webp` 仅采样脸部，卡通手使用 `cartoon-hand.js` 的手指、掌心和手腕关节。
- `assets/animal-rig.js`：整只动物的连续网格形变，保持头身轮廓和脚底位置；不支持 WebGL 时显示完整静态角色。
...
开发时运行 `python3 scripts/build-gallery.py` 同步合集预览；运行 `python3 scripts/package-site.py` 生成部署 ZIP。角色和闭眼表情由内置 imagegen 生成；卡通手为可独立弯曲手指的 SVG 控件。提示词记录位于 `assets/animal-art-prompt.md` 与 `assets/interaction-art-prompt.md`。
